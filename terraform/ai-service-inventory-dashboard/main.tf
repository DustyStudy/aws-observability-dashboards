...
data "archive_file" "inventory_collector" {
  ...
  output_path = "${path.module}/lambda/ai_service_inventory_collector.zip"
}
...
# Dashboard
# ---------------------------------------------------------------------------
resource "aws_cloudwatch_dashboard" "ai_service_inventory" {
    ...
    widgets = [
      {
        ...
        height = 6
        properties = {
          title  = "Active AI Services by Region (count of 6 watched services)"
      ...
      },
      {
        ...
        height = 6
        properties = {
          title  = "Bedrock — Active by Region"
          region = data.aws_region.current.name
          view   = "bar"
          metrics = [
            [{ expression = "SEARCH('{${var.metric_namespace},Service,Region} MetricName=\"ServiceActive\" Service=\"Bedrock\"', 'Maximum', 86400)", label = "Bedrock", id = "e1" }],
          ]
        }
      },
      {
        ...
        height = 6
        properties = {
          title  = "Bedrock Agents — Active by Region"
          region = data.aws_region.current.name
          view   = "bar"
          metrics = [
            [{ expression = "SEARCH('{${var.metric_namespace},Service,Region} MetricName=\"ServiceActive\" Service=\"BedrockAgents\"', 'Maximum', 86400)", label = "Bedrock Agents", id = "e1" }],
          ]
        }
      },
      {
        ...
        height = 6
        properties = {
          title  = "Bedrock Guardrails — Active by Region"
          region = data.aws_region.current.name
          view   = "bar"
          metrics = [
            [{ expression = "SEARCH('{${var.metric_namespace},Service,Region} MetricName=\"ServiceActive\" Service=\"BedrockGuardrails\"', 'Maximum', 86400)", label = "Bedrock Guardrails", id = "e1" }],
          ]
        }
      },
      {
        ...
        height = 6
        properties = {
          title  = "Rekognition — Active by Region"
          region = data.aws_region.current.name
          view   = "bar"
          metrics = [
            [{ expression = "SEARCH('{${var.metric_namespace},Service,Region} MetricName=\"ServiceActive\" Service=\"Rekognition\"', 'Maximum', 86400)", label = "Rekognition", id = "e1" }],
          ]
        }
      },
      {
        ...
        height = 6
        properties = {
          title  = "Comprehend — Active by Region"
          region = data.aws_region.current.name
          view   = "bar"
          metrics = [
            [{ expression = "SEARCH('{${var.metric_namespace},Service,Region} MetricName=\"ServiceActive\" Service=\"Comprehend\"', 'Maximum', 86400)", label = "Comprehend", id = "e1" }],
          ]
        }
      },
      {
        ...
        height = 6
        properties = {
          title  = "Textract — Active by Region"
          region = data.aws_region.current.name
          view   = "bar"
          metrics = [
            [{ expression = "SEARCH('{${var.metric_namespace},Service,Region} MetricName=\"ServiceActive\" Service=\"Textract\"', 'Maximum', 86400)", label = "Textract", id = "e1" }],
          ]
        }
      },
    ]
